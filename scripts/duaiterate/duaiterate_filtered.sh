...
NETWORK_NAME="berlin"
TRIPS_FILE="../../results/baseline-monday-berlin-sumo-checkpoint/trips.xml"

python3 ~/sumo/tools/assign/duaIterate.py \
    -n "${COMMON_PATH}${NETWORK_NAME}.net.xml" \
    -t "${TRIPS_FILE}" \
    --additional "${COMMON_PATH}gtfs_pt_stops.add.xml,${COMMON_PATH}validated.gtfs_pt_vehicles.add.xml" \
    --duarouter-additional-files "${COMMON_PATH}gtfs_pt_stops.add.xml,${COMMON_PATH}validated.gtfs_pt_vehicles.add.xml" \
